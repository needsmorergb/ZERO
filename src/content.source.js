(() => {
  ...
  const log = (...m) => EXT.DEBUG && console.log(`%c[ZERØ ${EXT.VERSION}]`, 'color: #14b8a6; font-weight: bold; background: #0d1117; padding: 2px 6px; border-radius: 3px;', ...m);

  // VERY VISIBLE version announcement
  console.log('%c ================================', 'color: #14b8a6');
  console.log(`%c 🎯 ZERØ Extension v${EXT.VERSION} LOADED 🎯`, 'color: #14b8a6; font-weight: bold; font-size: 16px; background: #0d1117; padding: 8px 16px; border-radius: 6px;');
  console.log('%c ================================', 'color: #14b8a6');
...
`;
  ...
  function getMarketCap() {
    ...
    try {
      if (PLATFORM.isPadre) {
        ...
        const mcElements = Array.from(document.querySelectorAll('h2.MuiTypography-h2, h2, h1, span, div'))
          .filter(el => {
  ...
  function getSolPrice() {
    ...
    const solImages = Array.from(document.querySelectorAll('img')).filter(img => {
      ...
      return (src.includes('solana') || src.includes('sol.') || src.includes('/sol') ||
        alt.includes('sol') || alt.includes('solana')) &&
        img.clientWidth < 50 && img.clientWidth > 8;
    });
    ...
    if (!price) {
      ...
      const solPriceMatch = allText.match(/SOL[:\s]*\$(\d{1,4}(?:,\d{3})*(?:\.\d{1,2})?)/i) ||
  ...
  function renderTradeMarkers() {
    ...
    log(`📍 renderTradeMarkers #${callId}: tokenKey="${tokenKey}", sessionRenderedMints has: ${[...sessionRenderedMints].join(', ') || 'none'}`);
    ...
    log(`📍 Marked ${tokenKey} as rendered. Session rendered mints: ${sessionRenderedMints.size}. lastRenderCallTs updated.`);
      ...
      root.addEventListener("click", async (e) => {
        ...
        if (act === "action") {
          ...
          if (BUYHUD_TAB === "buy") {
            ...
            const weightedEntryMC = oldSolSpent > 0
            ...
            if (oldSolSpent > 0 && marketCap < oldEntryMC) {
              ...
              if (pctDown < -5) { // Only warn if down more than 5%
