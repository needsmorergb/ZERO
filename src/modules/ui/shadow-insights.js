...
export const ShadowInsights = {
    ...
    _findBehavioralPattern(trades, sellTrades) {
        ...
        const winRate = sellTrades.length > 0 ? (wins.length / sellTrades.length * 100).toFixed(0) : 0;
    ...
    _findDisciplineBreak(trades, sellTrades) {
        ...
        const noStrategy = trades.filter(t => t.side === 'BUY' && (!t.strategy || t.strategy === 'Unknown' || t.strategy === 'Other'));
        if (noStrategy.length > 0 && noStrategy.length >= trades.filter(t => t.side === 'BUY').length * 0.4) {
