...
export const NarrativeTrust = {
    ...
    _buildSignals(context, social) {
        ...
        const activityStatus = (enrichStatus === FIELD_STATUS.OK && accountAge != null)
            ? (accountAge > 30 ? 'established' : 'new')
            : social?.activityBucket === 'recent' ? 'detected'
            : social?.activityBucket === 'stale' ? 'not_detected'
            : 'unavailable';

        // X Communities: from context API
        const xCommStatus = context?.x?.communities?.status;
        const xCommSignal = xCommStatus === FIELD_STATUS.OK ? 'detected'
            : xCommStatus === FIELD_STATUS.NOT_SUPPORTED ? 'unavailable'
            : xCommStatus === FIELD_STATUS.MISSING_IDENTIFIER ? 'not_detected'
            : 'unavailable';

        // Developer: depends on API availability
        const devStatus = context?.dev?.status === FIELD_STATUS.OK ? 'detected'
            : context?.dev?.status === FIELD_STATUS.NOT_SUPPORTED ? 'unavailable'
            : 'not_detected';
    ...
    _calculateScore(context, social) {
        ...
        const enriched = context?.x?.profile?.enrichmentStatus === FIELD_STATUS.OK
