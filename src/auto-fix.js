...
(function () {
  ...
  console.log(
    `%c 🎯 auto-fix.js v${VERSION} LOADED 🎯`,
    "color: #f59e0b; font-weight: bold; font-size: 14px; background: #0d1117; padding: 8px 16px; border-radius: 6px;"
  );
  console.log("%c ================================", "color: #f59e0b");
  ...
  function processPendingMarkers() {
    ...
    const tvVars = Object.keys(window).filter(
      (k) =>
  ...
  const getWidgetInstance = () => {
    ...
    try {
      ...
      console.log("[paper-fix] Error searching iframes:", e);
    }
  ...
  function createMarkerNowWithWidget(widget, side, price, marketCap, timestamp) {
    try {
      ...
      try {
        ...
        /* font method may not exist */
      }

      console.log(
        "[paper-fix] ✓ TradingView marker created:",
        shape,
        "at time:",
        ts,
        "price:",
      ...
      try {
        ...
        if (chart && chart !== lastChartRef) {
          if (lastChartRef !== null) {
            console.log(
              `[auto-fix v${VERSION}] ♻️ Chart instance changed! Clearing marked trade IDs to allow re-rendering.`
            );
  ...
  window.addEventListener("message", function (event) {
    ...
    if (msg.__paper === true) {
      ...
      if (msg.type === "RENDER_STORED_MARKERS" && Array.isArray(msg.trades)) {
        ...
        console.log(
          `[auto-fix v${VERSION}] 📋 RENDER_STORED_MARKERS: ${msg.trades.length} total, ${newTrades.length} new`
        );
        ...
        const tryRender = (attempts = 0) => {
          ...
          if (widget) {
            console.log(
              `[auto-fix v${VERSION}] ✅ Widget ready, rendering ${newTrades.length} new markers`
            );
            let renderedCount = 0;
            ...
            console.log(
              `[auto-fix v${VERSION}] ✓ Rendered ${renderedCount} unique markers (from ${newTrades.length} candidates)`
            );
          } else if (attempts < 10) {
  ...
  const tryStartMonitoring = () => {
    ...
    if (PLATFORM_NAME === "Padre") {
      ...
      const hasBuyButton = Array.from(document.querySelectorAll("button")).some((button) =>
        (button.textContent || "").trim().toLowerCase().includes("buy")
      );
      isReady =
        document.querySelector("canvas") ||
